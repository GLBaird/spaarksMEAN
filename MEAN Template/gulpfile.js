 ...
 *      gulp build:test (default build preceded with defined unit tests through KARMA)
 ...
 *      gulp build:final:test (Same as build final but preceded with unit testing)
 ...
 *      gulp help (prints this comment to the console)
 *      gulp test (performs specified unit test with KARMA)
...
// replacements for index.html final build
var indexFinalBuildRaplacements = [
    { pattern: /src=["'].+?\/angular.+?.js["']/i,
      replace: "src='https://ajax.googleapis.com/ajax/libs/angularjs/1.4.7/angular.min.js'" },
...
// prints HELP information in terminal
gulp.task('help', function() {
    console.log("\n\n* STANDARD GULP BUILD TASKS");
    console.log("* ------------------------------------------------------------------------------------------");
    console.log("* Gulp bulk Task runners:");
    console.log("*      gulp                    default build");
    console.log("*      gulp build              default build");
    console.log("*      gulp build:test         default build preceded with defined unit tests through KARMA");
    console.log("*      gulp build:clean        default build with clean");
    console.log("*      gulp build:min          default build with minified css and js");
    console.log("*      gulp build:clean:min    default build with clean and minifed css and js");
    console.log("*      gulp build:final        outputs final production build:");
    console.log("*                              cleans build, copies and only produces minified");
    console.log("*                              and concatinated/browserfied css and js");
    console.log("*      gulp build:final:test   Same as build final but preceded with unit testing");
    console.log("*      gulp watch              watch making css and js builds only and individually,");
    console.log("*                              with no minifaction, fastest build times");
    console.log("*      gulp watch:copy         watch making css and js build builds only and");
    console.log("*                              individually, running the copy script on each build,");
    console.log("*                              but with no minifaction");
    console.log("*      gulp watch:clean        watch making default build, with clean and");
    console.log("*                              no minifaction");
    console.log("* ------------------------------------------------------------------------------------------");
    console.log("* Gulp Individual tasks");
    console.log("*      gulp clean              erases marked folder - default 'dest'");
    console.log("*      gulp test               performs specified unit test with KARMA");
    console.log("*      gulp copy               copies marked files from src to dest");
    console.log("*      gulp css                concatinates all css into 'css/styles.css'");
    console.log("*      gulp css:min            concatinates and minifies into 'css/styles.min.css'");
    console.log("*      gulp browserify         concatinates and browserify all js to 'js/main.js'");
    console.log("*      gulp browserify:min     concatinates, browserify and uglify all js to 'js/main.min.js'\n\n");
});
...
// run karma unit testing through browserify
...
// copy files
// wait for clean to complete - Copy Final does not copy any index.html files
gulp.task('copy:final', dependentTasks, function() {
    copyFiles.forEach(function (file) {
        if (file.src.indexOf("index.html")<0) {
            gulp.src(file.src)
                .on('error', function (err) {
                    console.log(err.toString());
                    this.emit("end");
...
gulp.task('buildFinalIndex', dependentTasks, function() {
    // find index file in copy list
    copyFiles.forEach(function(file) {
        if (file.src.indexOf("index.html") >= 0) {
            // process file with replacement instructions
            var bundle = gulp.src([file.src]);
            for (var i in indexFinalBuildRaplacements) {
                if (indexFinalBuildRaplacements.hasOwnProperty(i)) {
                    var replacement = indexFinalBuildRaplacements[i];
                    bundle.pipe( replace(replacement.pattern, replacement.replace) );
                }
            }
            bundle.pipe( gulp.dest(file.dest) );
            return false;
        }
    });
});
...
gulp.task('build:final', ['clean', 'copy:final', 'buildFinalIndex', 'css:min', 'browserify:min']);
gulp.task('build:final:test', ['test', 'clean', 'copy:final', 'buildFinalIndex', 'css:min', 'browserify:min']);
